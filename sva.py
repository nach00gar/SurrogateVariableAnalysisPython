...
def batchEffectRemoval(expressionMatrix, labels):
    if len(np.unique(labels)) == 2:
        mod = np.vstack((np.ones(labels).squeeze(), pd.get_dummies(labels, dtype="int").iloc[:, 1])).T
    else:
    ...
    res = irwsva_build(dat, mod, mod0, n_sv, B=5)

    ndb = dat.shape[1]
    nmod = mod.shape[1]
    mod1 = np.hstack([mod, res["sv"]])

    gammahat = (dat @ mod1 @ np.linalg.inv(mod1.T @ mod1))[:, nmod:(nmod + n_sv)]
    expressionMatrixBatchCorrected = dat - gammahat @ res["sv"].T

    return expressionMatrixBatchCorrected


def num_sv(expressionMatrix, mod):
    def row_vars(matrix):
        return np.var(matrix, axis=1, ddof=1)

    # Asumimos que dat es un array de NumPy
    dat = expressionMatrix.to_numpy()
    dims = dat.shape
...
def edge_lfdr(p, trunc=True, monotone=True, adj=1.5, eps=1e-8, lambda_val=0.8):
    ...
    p = np.clip(p, eps, 1 - eps)  # Ensure p is within (eps, 1-eps)
    x = norm.ppf(p)
    bw = bw_nrd0(x)
...
def irwsva_build(dat, mod, mod0=None, n_sv=None, B=10):
    ...
    # Ordena los índices de los eigenvalores en orden descendente
    sorted_indices = np.argsort(eigenvalues)[::-1]

    # Ordena los eigenvalores y los eigenvectores usando los índices ordenados
